    ...
    with open(filename, 'r') as file:
        ...
        for row in reader:
            evidence_row = [
                ...
                float(row[8]), float(row[9]), month_dict[row[10]],
...
def evaluate(labels, predictions):
    ...
    # sum(labels) represent all positive labels
    sensitivity = true_positive / sum(labels)

    all_negative_labels = len(labels) - sum(labels)
    specificity = true_negative / all_negative_labels
